...
class LidarSegmentation(object):
    ...
    def create_graph(self, lidar, projected, n_rows, n_cols):
        ...
        # S = cp.sparse.coo_matrix((cp.array(d), (cp.array(row_indices),
        #                                         cp.array(col_indices))),
        #                          shape=(n_points+n_pixels, n_points+n_pixels))
    ...
    def run(self, lidar, detections, max_iters=200, device=0, save_all=True):
        with cp.cuda.Device(device):
            ...
            # all_label_likelihoods = [initial_lidar_labels]
            # all_label_likelihoods = cp.empty((max_iters+1, n_points, n_instances+1))
            if save_all:
            ...
            # all_label_likelihoods[0,:,:] = cp.array(initial_lidar_labels)
            # all_label_likelihoods[0,:,:] = initial_lidar_labels

            for i in range(max_iters):
                Y_new = G_gpu.dot(Y_gpu)
                # if cp.allclose(Y_new, Y_gpu): # THIS IS VERY SLOW
                #     print("Converged at iter %d" % i)
                #     break
                Y_gpu = Y_new
                # all_label_likelihoods.append(self.class_mass_normalize(Y_gpu[:n_points,:].get(), detections))

                # Note: for performance, should later turn off saving all iterations
                if save_all:
            ...
            # all_label_likelihoods[-1,:,:] = self.class_mass_normalize(all_label_likelihoods[-1,:,:], detections)

            # all_label_likelihoods = np.array(all_label_likelihoods)
            # all_label_likelihoods = cp.asnumpy(all_label_likelihoods)
